...
var AjouMSGRecevier = function (solaceModule,sessioninfo) {
  ...
  ajou.connect = function () {
      ...
      ajou.session.on(solace.SessionEventCode.MESSAGE, function (message) {
          ...
          console.log(text);
          const User = DBData.User[Math.floor(Math.random()*29)];
          const Ride = DBData.Ride[Math.floor(Math.random()*5)];
          const start = LocationData[Math.floor(Math.random()*9)];
          const dest = LocationData[Math.floor(Math.random()*9)];
          let taximove = Math.floor(distance(start.lat,start.lon,dest.lat,dest.lon));
          taximove = 0;
          console.log(User.Name+"의 택시에서 " + Ride.Name+"승객이 하차하였습니다.");
          console.log(start.area + "에서 " + dest.area + "까지 이동하였습니다.");
          if(taximove==0)
            taximove=10;
          console.log("거리는 총 " + taximove +"km이며, 비용은 " + taximove*2500 +"원 입니다.");
          ...
          queueinfo.message = JSON.stringify(messageJson); //Cost 및 기타 내용
          //타입: MsgPub, MsgSub, QuePro, QueCom
          queueinfo.type = messagetype[2];
          ajou.run(queueinfo);
          setTimeout(function () {
          queueinfo.queueName = business[6];
          ajou.run(queueinfo);
          }, 500);
      });
      // connect the session
          ...
          if (ajou.consuming) {
              ...
              try {
                  ...
                  ajou.messageConsumer.on(solace.MessageConsumerEventName.MESSAGE, function (message) {
                    ...
                    //       ' details:\n' + message.dump());
                      // DropoffComplete/Timestamp/Ride-ID/Location/User-ID
                      //        0            1        2       3        4
                      var Dropoffdata = message.getDestination().name.split('/');
                      const User = DBData.User[Math.floor(Math.random()*29)];
                      const Ride = DBData.Ride[Math.floor(Math.random()*5)];
                      const start = LocationData[Math.floor(Math.random()*9)];
                      const dest = LocationData[Math.floor(Math.random()*9)];
                      const taximove = Math.floor(distance(start.lat,start.lon,dest.lat,dest.lon));
                      console.log(User.Name+"의 택시에서 " + Ride.Name+"승객이 하차하였습니다.");
                      console.log(start.area + "에서 " + dest.area + "까지 이동하였습니다.");
                      if(taximove===0)
                        taximove=10;
...
const LocationFile = fs.readFileSync('./LocationDB.json', 'utf8');

const DBData = JSON.parse(DBFile);
const LocationData = JSON.parse(LocationFile);

const messagetype = ['MsgPub', 'MsgSub', 'QuePro', 'QueCom'];
const business = ['Samsung', 'Apple', 'Qualacomm', 'Tesla', 'Intel', 'Coke','Individual'];
...
var connectinfo = {};
  connectinfo.protocol =  'ws://mr-connection-r9c061sbgi4.messaging.solace.cloud:80';
  connectinfo.clientname = 'solace-cloud-client'; 
  connectinfo.vpn = 'trial2';
  connectinfo.password = '8v50dqq7v278p98hrnr0l7qj33';
...
var changeinfo = {};
  //MsgPub, MsgSub
  changeinfo.topic = 'DropoffComplete/>';
  //QuePro, QueCom
